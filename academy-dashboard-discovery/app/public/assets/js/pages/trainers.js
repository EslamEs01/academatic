/* Trainers page — admin directory (card grid). NOT a teacher dashboard/portal. */
import { TRAINERS, TRAINER_AVAIL } from '../fixtures/trainers.js';
...
function card(tr) {
  const av = TRAINER_AVAIL[tr.avail];
...
function preview(tr) {
  ...
  const body = `
    ...
    <p class="text-[13px] mb-4" style="color:var(--c-ink-2)">${tr.bioKey ? t(tr.bioKey) : t('data.trn.bioSara')}</p>
...
export function renderTrainers() {
  ...
  const filters = filterBar({
    targetId: 'trainers-grid', searchKey: 'trn.searchPh',
    selects: [
      { name: 'availability', labelKey: 'trn.fAvail', options: [{ value: 'all', labelKey: 'filter.all' }, ...Object.keys(TRAINER_AVAIL).map((v) => ({ value: v, labelKey: TRAINER_AVAIL[v].labelKey }))] },
  ...
  return `
    ...
    ${filters}
    ${cardGrid(rows.map(card), { cols: 'sm:grid-cols-2 xl:grid-cols-3', id: 'trainers-grid' })}
    ${noResults()}
