...
/* generic status chip for non-session statuses (student/trainer/course) */
export function chip({ labelKey, label, tone = 'neutral', icon: ic } = {}) {
  const lead = ic ? icon(ic, 'ico') : '';
  return `<span class="chip tone-${tone}">${lead}<span>${labelKey ? t(labelKey) : esc(label)}</span></span>`;
}
