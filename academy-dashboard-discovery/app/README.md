...
## Spec 035 — Families & Students Nav Completion (Family Categories / Schedule Search / Student Results / Student Evaluation)

The second Spec-033-roadmap follow-up: it closes the four remaining **Families**-category «قريبًا» items. **Count 113 → 115 (+2)** — exactly one new page base. The four resolutions use the honesty-first rule (show the surface, gate only the final backend action):

- **familyCategories → fold-anchor** to `families.html` (count 0). The families list already carries a category filter + the display-only `fam-cat` reclassify drawer (gated "Save category"); the nav item becomes a real `<a>` to that owner instead of a dead «قريبًا». No body edit, no fake category create/save/mutation.
- **scheduleSearch → new standalone page** `schedule-search.html` (+ `.en`), grounded in the legacy `management/search-schedule` availability finder: KPI strip + a teacher/subject/day/time-window/availability filter bar + an authored results board (teacher + subject + day + time + room + a labeled availability chip) + per-slot read-only detail drawers + an empty state. Filtering is the existing client-side `data-filter`/facet mechanism over authored fixture rows (**no availability engine, no network, no pay figure**); **Book/Assign = `backendRequired` gates** (no fake booking/assignment/mutation).
- **studentResult → deep-link** `student.html#view=results` (count 0) — the existing display-only Results tab (per-course progress + certificates; **no gradebook/computed score**).
- **studentEvaluation → deep-link** `student.html#view=evaluation` (count 0) — the existing display-only Evaluation rubric tab (categorical ratings; Approve stays a `backendRequired` gate; **no computed total**).

**Mechanism**: existing primitives only (pageHeader/summaryCards/filterBar/facetAttrs/previewTemplate/chip/button/noResults) + the CLOSED `data-*` hook set; one new authored fixture (`fixtures/schedule-search.js`) + one new mirrored locale pair (`ar/en.ssr.js`, registered in `i18n.js` → 13 pairs). `sidebar.js` `langRoute()` is made **hash-aware** (backward-compatible) so EN deep-links resolve to `student.en.html#view=…`. **No `package.json` change, no dependency, no backend/API/engine, `enhance.js` 0-diff, no `app.css` change.**

**Django note**: schedule-search → an availability query over `Session`/`Teacher` timetables rendered as a results list (Book/Assign post, gated until the backend is wired); familyCategories → the `families` list route (its category filter + reclassify form); studentResult/studentEvaluation → `student/<id>?view=results|evaluation` — the same display-only tabs, no gradebook.

**Verified**: build 115; smoke PASS (additive Families/Students block — route-freeze 115, the 4 nav-flip anchors + families 0-planned + admin-menu 50, schedule-search form/results/gates/empty-state/facet-narrow + 0 external request + no pay/`type=file`/`type=password`/`<canvas>`, student `#view=results`/`#view=evaluation` deep-links open the right tab; protected role-law + Spec-032 + 026–034 asserts BYTE-VERBATIM; sanctioned amendments: route-freeze 113→115 + the dashboard planned-item probe repointed families→teachers); a11y critical=0 serious=0 (+schedule-search light/dark/mobile-390/open-drawer rows); screenshots 0 console errors (+11 sp035 frames). Only the 52 admin pages' shared sidebar changed (4 «قريبًا» → anchors — PROVEN: `#page-body`-onward byte-identical for families/family/students/student ×2 lang); all portal pages + index byte-identical; `package.json`/`enhance.js` 0-diff.

## Spec 036 — Teachers Nav Completion (Add Teacher / Teacher Categories / Sessions KPI / Monthly Performance)

The third Spec-033-roadmap follow-up: it closes the four remaining **Teachers**-category «قريبًا» items. **Count held 115 → 115 (0 new pages).** Bound by the GLOBAL teacher pay-free law — no salary/rate/fine/payout/currency figure on any touched surface.

- **addTeacher → fold-anchor** to `teachers.html` (count 0). The teachers page already hosts the `trn-add` form drawer (names/email/phone/status/subjects/level/courses/city/country/notes + a CV **gate**; the legacy Salary/Payout/Zoom/password sections stay omitted forever). The nav item becomes a real `<a>`; Save stays a `backendRequired` gate.
- **teacherCategories → fold-anchor** to `teachers.html` (count 0). Reuses the existing `trn-categories` drawer (display-only category list + inline Create form + Save/assign gates); the stale `FUTURE_ROUTES.teacherCategories` is dropped.
- **sessionsKpi → display tab** `teacher-performance.html#view=sessions-kpi` (count 0). `teacher-performance.html` gains a `tabs()` widget (the existing board becomes the **Overview** tab); the new Sessions-KPI tab shows per-teacher authored session **counts** (`teacherCounts`) + a categorical quality chip. **No computed percentage/score/rank/chart** — the legacy "Classes KPI" `Percentage` is deliberately not reproduced.
- **monthlyPerf → display tab** `teacher-performance.html#view=monthly` (count 0). A monthly board grouped by month: per-teacher categorical trend chip + authored note. **No computed figure**; does not duplicate the Spec-029 feedback engine.

**Mechanism**: existing primitives only (`tabs()` + `#view=` hash · `cardGrid`/`statMini`/`chip`/`avatar` · the `trn-add`/`trn-categories` drawers) + one new authored fixture (`fixtures/teacher-performance.js`) + new keys in the existing mirrored `ar/en.trn.js` (`trn.board.tab.*` · `trn.sessKpi.*` · `trn.monthly.*`; the `trn.kpi` profile block is untouched). **No `package.json`/dependency/backend/API/engine; `enhance.js` 0-diff; `build-html.mjs` 0-diff (no new page); `i18n.js` 0-diff (no new locale module).** The two new tabs are static display boards (no filterBar) to honor enhance.js's single global `[data-no-results]` contract without touching it.

**Django note**: teacher-performance → one view with `?view=overview|sessions-kpi|monthly` tabs over the same authored teacher context (counts + categorical labels, no computed metric); addTeacher/teacherCategories → the `teachers` list route (its `trn-add`/`trn-categories` `ModelForm`s, gated until the backend is wired). No pay model on any teacher surface.

**Verified**: build 115; smoke PASS (additive Teachers block — 4 nav-flip anchors + teachers 0-planned + admin-menu 50, teacher-performance 3-tab widget + no computed score/rank/percentage/chart + no pay, `#view=sessions-kpi`/`#view=monthly` deep-links open the right tab on fresh load, `trn-add`/`trn-categories` reachable+gated; protected role-law + teacher-pay + Spec-032 + 026–035 asserts BYTE-VERBATIM; sanctioned amendment: the dashboard planned-item probe repointed teachers→admin, route-freeze stays 115); a11y critical=0 serious=0 (+teacher-performance `#view=sessions-kpi`/`#view=monthly` light/dark/mobile-390 rows); screenshots 0 console errors (+9 sp036 frames). `teachers.html`/`teacher.html` `#page-body` byte-identical (fold anchors = nav-only, proven); only `teacher-performance.html`/`.en` bodies change (tabs) + the shared sidebar; all portal pages + index byte-identical; `package.json`/`enhance.js`/`build-html.mjs`/`i18n.js` 0-diff.

## Spec 037 — Reports / Analytics Nav Completion + Missing-Pages Correctives

The fourth Spec-033-roadmap follow-up: it closes the two remaining **Reports/Analytics** «قريبًا» items **and** strengthens the three Spec-035 items the maintainer flagged as "still missing". **Count held 115 → 115 (0 new pages; admin-menu 50).** A full Admin missing-pages audit (all 50 items) found 0 truly-missing / 0 ownerless targets.

- **monthlyReports → display tab** `reports.html#view=monthly` (count 0). `reports.html` gains a `tabs()` widget (the existing page becomes the **Overview** tab, byte-identical — the 7 `.report-card`s preserved); the Monthly tab shows authored month-grouped rows (area + count literal + categorical status chip) + summary cards. **No computed metric/percentage/chart, no money figure.**
- **dataAnalysis → display tab** `reports.html#view=analysis` (count 0). Authored insight cards + **AUTHORED categorical trend labels** (improving/steady/declining — not computed). Finance-flavoured analysis (expenses/invoices) stays excluded → Spec 038, keeping the reports body finance-free.
- **familyCategories → labeled Categories board** `families.html#view=categories` (count 0). `families.html` gains a Directory + Categories `tabs()` widget; the Categories tab lists the authored `FAMILY_CATEGORIES` (name + description + authored member `count` + status chip) with the existing `fam-cat` reclassify drawer reachable + a Create-category `backendRequired` gate. No computed statistic, no fake mutation.
- **studentResult / studentEvaluation → cross-student boards** `students.html#view=results` / `#view=evaluation` (count 0). `students.html` gains a Directory + Results + Evaluation `tabs()` widget; each board lists students with authored fields (level + certificate-count literal + lifecycle/approval status chip) and a **per-student deep-link** to the UNCHANGED single-student `student.html#view=results`/`evaluation` tabs. **No computed score/rank/GPA/percentage/rubric-total.**

**Nav**: 2 flips (monthlyReports/dataAnalysis `planned → implemented`) + 3 route refinements (familyCategories/studentResult/studentEvaluation) + `FUTURE_ROUTES` trim of `monthlyReports`/`dataAnalysis` only. Reports category now 0 «قريبًا»; admin-menu 50.

**Mechanism**: existing `tabs()` + `#view=` fold ONLY. The new tabs are **pure display boards** (no filterBar) to honor enhance.js's single global `[data-no-results]` contract without touching it (each page keeps its one existing filterBar in the Overview/Directory tab). New authored data in `fixtures/reports.js` (`MONTHLY_REPORTS`/`MONTHLY_SUMMARY`/`DATA_INSIGHTS`); `FAMILY_CATEGORIES` + per-student `results`/`evaluation` reused as-is; new keys in the existing mirrored `ar/en.rep.js` + `ar/en.fam.js`. **No `package.json`/dependency/backend/API/engine; `enhance.js` 0-diff; `build-html.mjs` 0-diff (no new page); `i18n.js` 0-diff (no new locale module).**

**Verified**: build 115; smoke PASS (additive Reports/Analytics + correctives block — nav037 anchors + reports 0-planned + reports/families/students tab widgets + overview 7-card preserved + no computed/canvas/money in the new boards + per-student deep-links + the 5 `#view=` deep-links open on fresh load AR/EN; the nav035 route asserts updated to the refined routes — sanctioned amendment; protected role-law/finance/Spec-032 + 026–036 asserts BYTE-VERBATIM); a11y critical=0 serious=0 (+16 rows); 321 screenshots 0 console errors (+19 sp037 frames). Locale parity `rep` 251/251 · `fam` 393/393 (0 divergence). `student.html`/`family.html` + `result-summary`/`evaluation-rubric` byte-identical; only `reports`/`families`/`students` bodies (×2 lang) + the shared sidebar changed; all portal pages + index byte-identical; `package.json`/`enhance.js`/`build-html.mjs`/`i18n.js` 0-diff.

## Spec 038 — Finance Nav Completion (Invoices / Payments / Monthly Invoices / Salaries / Staff Salaries / Class Salary Report / Banks)

The fifth Spec-033-roadmap follow-up (on a **committed** baseline — HEAD `56bc418` bundled Specs 035/036/037). It completes the **Finance** sidebar by growing `finance.html` from **3 → 6 tabs** and unlocking six locked nav items — under the GLOBAL finance no-fake-money law. **Count held 115 → 115 (0 new pages; admin-menu 50).**

- **invoices → `finance.html#view=invoices`** (count 0). The invoice tiles + filterBar + the single `#invoice-list` (9 authored rows, amount literals + status chips) are **MOVED** (not duplicated) from Overview into a focused Invoices tab. Create/Generate/PDF/Send/Mark-Paid = `backendRequired` gates; **no computed total**.
- **payments → `finance.html#view=payments`** (count 0). The single 6-row `.fin-pay-row` list is MOVED into a Payments tab; serial links open the existing `inv-*` drawer; Record/Confirm/Refund/Export = gates; no settlement/gateway/mutation.
- **monthlyInvoices → `finance.html#view=monthly-invoices`** (count 0). A NEW **derived** board grouping the existing 9 INVOICES by authored `monthKey` (4 month groups) using `.finm-*`/`#fin-monthly` (never `.fin-row`/`.report-card`), per-row amount literals + a per-month **count** label — **no computed monthly total**; Generate/Send/Export = gates; **`fixtures/finance.js` 0-diff**.
- **salaries + staffSalaries → `finance.html#view=salaries`** (count 0). Both unlock to the existing **figure-free** teacher + staff status boards (name+status+period, **no pay amount**); Generate/Approve/Mark-paid/Export = gates.
- **banks → `finance.html#view=banks`** (count 0). Existing name+status board (**no balance**); Add/Import/Reconcile = gates.
- **classSalaryReport → honest lock kept** (`disabled` + `nav.reason.finance` + lock; no route). A real class-salary report implies computed per-class pay (forbidden) → owner = future backend billing/accounting spec.
- **finance-analysis → deferred** (no nav item/route). analysis-expenses/analysis-invoices imply computed profit/loss/revenue/VAT → represented by the `accountingExpenses` planned card; owner = future backend spec.

**Overview** keeps `financeActions()` (the first `.report-actions`, ≥4 gates) + the **9 figure-free planned cards** + the **9 baked `inv-*` drawers** (baked once, cloned from any tab). **Nav**: 6 unlocks `disabled → implemented`; classSalaryReport unchanged; no FUTURE_ROUTES edit; admin-menu 50.
