...
## Displacement re-homing — delivered by Spec 020 (recorded 2026-07-05)

The Spec-018 displacement map's FAMILY rows are now re-homed (the RETAINED fixtures/keys render again; none retired): the children cards → `family-children` (as the full directory; the home keeps its compact five) · today-across-children → `family-schedule` · attendance signals + teacher notes + history framing → `family-progress` · subscriptions + billing status (`billSettled`/`billReassure`/`subsPlan` + the billingGate) → `family-billing` (joined by the hour-quota idea and amount-free invoice rows) · the requests hub (`prt.fam.req.*` complete: trial/meeting/feedback/cancel + caution) → `family-requests` · materials + matDownload → `family-materials` (extended to cover st12/st13) · the account register (`prt.fam.acct.*`) → `family-profile` (with the three profile-edit gates). The family home stayed compact; its quick-tiles became real links (the D13/019 honesty law); `family-child` body BYTE-EQUAL (extraction-proven). The remaining displaced rows belong to Spec 021 (teacher).
