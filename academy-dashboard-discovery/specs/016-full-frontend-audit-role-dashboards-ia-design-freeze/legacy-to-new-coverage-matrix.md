...
**Spec 018 (2026-07-04) — Role Dashboards Admin-Like UX Rework: DELIVERED (corrective; zero coverage loss).** The three role HOME rows are now COMPACT admin-like dashboards (7 bands, hard ceiling) and gain the NEW `family-child(.en).html` drill-down (the guardian→child-profile capability the legacy app demanded — its legacy profile page returned a **500** — now delivered for the real fam1 roster st1/st6/st11/st12/st13). **No matrix row changes status**: every capability displaced from the long homes is RETAINED in fixtures/locales (grep-audited) and stays owned by its future page row (student→019, family→020, teacher→021). The sequence renumber (018 corrective · 019–021 role-internal · 022–027 admin · 028 QA) is recorded append-only in `future-spec-sequence.md`. The legacy "Select-Student → Submit" child-history filter is the direct ancestor of the family-child drill-down (rebuilt as five real deep links, not a form). Visual grounding recorded in `specs/018-.../visual-grounding-addendum.md` (23 frames).

**Spec 019 (2026-07-04) — Student Internal Pages: DELIVERED.** The Student Dashboard App is now frontend-complete: the six planned student nav entries flipped `planned → implemented` and their pages ship (`student-schedule/homework/materials/progress/history/profile` ×AR/EN → 51→63 built, 49/63 hash-identical). The STUDENT section's "net-new value beyond legacy" claim is realized against the guardian-proxy evidence (no legacy student role existed — grounding recorded in `specs/019-…/visual-grounding.md`): **schedule** ← the guardian timetable (day-grouped agenda replaces the empty grid) · **homework** ← the per-session homework-note record + teacher task taxonomy (net-new first-class surface) · **materials** ← the guardian/teacher library (hero anti-pattern removed; per-course groups) · **progress** ← re-homes the retained 013 achievements/celebration + course bars (charts stay admin-side, per `management-analysis-student-full`) · **history** ← the guardian history-filter + teacher course-history record shape (out1 real ref) · **profile** ← the profile-edit page's exact write surface, delivered as the three backendRequired gates (photo/save/password; the legacy view page was a 500). Every write action is a labeled gate — zero fake save/upload/live. Displacement law honored: the 018-displaced student fixtures/keys are RETAINED and now RENDER again on their owning pages.

**Spec 020 (2026-07-05) — Family / Guardian Internal Pages: DELIVERED.** The Family Dashboard App is frontend-complete: the seven planned family nav entries flipped `planned → implemented` and their pages ship (`family-{children,schedule,progress,billing,requests,materials,profile}` ×AR/EN → 63→77 built, 59/77 hash-identical), with `family-child` preserved as the drill-down (body byte-equal, extraction-proven). Every §8 F-row resolves against the 100%-inspected guardian evidence (27/27 frames): **children** ← "All Account Subscriptions" (every legacy column → a card element or the real `#child=stX` drill-down) · **schedule** ← the child-tagged today table + week grid (rebuilt as tagged cards + day-grouped agenda) · **progress** ← the multi-child Classes-Summary select (rebuilt as five per-child cards w/ real drill-downs; no legacy progress page existed — authored figures) · **billing** ← the hour-quota home banner + the invoice table WITH ITS AMOUNT COLUMN DELIBERATELY DROPPED (status-first: serial/month/due/course/status; the payFigure regex now guards all seven pages) · **requests** ← the meetings table + the trial wizard (step-2 stays gated — the recorded gap) + the 014 cancel/feedback registers re-homed · **materials** ← the category library minus the hero, regrouped by child (all five covered) · **profile** ← the profile-edit write surface as the three backendRequired gates (the legacy view page was a 500). Zero capability deletion; the RETAINED 014/018 registers now RENDER again on their owning pages.
