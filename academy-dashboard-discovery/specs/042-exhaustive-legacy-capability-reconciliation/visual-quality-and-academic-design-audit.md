...
> **Reconciliation note (post-drafting).** The rows tagged `[C07]` / `[C12]` / `[C13]` / `[C15]` were drafted
> while those four cluster audits were still in flight; all four have since landed
> (`cluster-audits/{C07,C12,C13,C15}-audit.md`). Where this document cites them, the audits' own disposition
> tables are authoritative for row IDs and verdicts — the key confirmations: the staff RBAC drawer
> never-rendered finding = C12 audit §5 (folded into **C12-20**); the finance-hub visual rows = **C07-32/C07-33**;
> the mobile-drawer clip = **C15-15**; the teacher quick-tiles «قريبًا» defect = **C15-14**; the teacher
> monthly-report / results read-only rows = C13 audit §2 (+ the C08-02 rubric-authoring reference). Any residual
> per-row wording difference resolves to the cluster audits, not to this document.
...
| **V-A6** | **The KPI tiles and status tiles are inert.** In the legacy, every home tile is a drill-down (`?status=1\|8\|2,10\|...`). Ours are cheerful dead ends. | `app/src/js/components/status-tile.js` (no href/hook); legacy `output/roles/admin/pages/management-home.json` (status query variants) | **MEDIUM** — recorded as `C01-01` PARTIAL | **047** |
...
| `family-progress` | POLISH | The consumer of a report **nobody in the product can author** (C08-02) — visually complete, structurally orphaned → **056** + **055**. |
