  ...
  return `<header class="topbar">
    ...
    <div class="topbar-search">
      <div class="input-wrap w-full">
        ...
        <input class="input" type="search" placeholder="${esc(t('topbar.searchPlaceholder'))}" aria-label="${esc(t('topbar.searchPlaceholder'))}" data-action="command-palette" aria-haspopup="menu" />
        <span class="kbd-hint"><kbd class="kbd" dir="ltr">⌘K</kbd></span>
      </div>
    </div>

    <div class="util-cluster">
      <button type="button" class="icon-btn" data-action="apps-grid" aria-label="${esc(t('topbar.apps'))}" aria-haspopup="menu">
        ${icon('grid', 'ico')}
      </button>
      <button type="button" class="icon-btn" data-action="quick-actions" aria-label="${esc(t('topbar.quickActions'))}" aria-haspopup="menu">
        ${icon('plus', 'ico')}
      </button>
